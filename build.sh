...
unzip -q "dist/$filename" -d "eden"

# Move the 'publish' directory to 'eden'
# mv ./eden/publish ./eden

# Create 'user' directory inside 'eden'
mkdir ./eden/user

# Change to the 'eden' directory
cd eden

# Run eden.exe in the background and capture its PID
# Note: If running on Linux, you may need Wine (e.g., 'wine ./eden.exe &')
...
cp -r ../ProdKeys_21/Prodkeys.NET_v21-0-0/*.keys ./user/keys
cp -r ../Firmware_21/* ./user/nand/system/Contents/registered

# rename Firmware
for file in ./user/nand/system/Contents/registered/*; do
    nca=$(basename "$file")
    ...
    folder="./user/nand/system/Contents/registered/$xxx.nca"
    mkdir "$folder"
...
# add games dir
# sed -i 's/"game_dirs": \[\]/"game_dirs": ["user\/games"]/g' ./user/Config.json
